  ...
  return (
    <header className="flex h-14 flex-none items-center justify-between border-b border-border bg-background/70 px-6 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-2.5 rounded-[10px] py-1 pl-1 pr-2 transition hover:bg-secondary">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-forest-soft font-display text-[12px] font-semibold text-forest">
            {initials(SCHOOL.principal)}
          </span>
          <span className="hidden text-left leading-tight sm:block">
            <span className="block text-[13px] font-semibold text-ink">{SCHOOL.principal}</span>
            <span className="block text-[11px] text-ink-4">Principal · {SCHOOL.shortName}</span>
          </span>
